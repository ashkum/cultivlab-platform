...
How to open `https://chat.${DOMAIN}`, log in with the provided credentials, and start a
conversation. How to pick a model (Claude, ChatGPT, Gemini).
...
Download link, installation steps for Mac, Windows, and Linux. Verification step.
...
Installing Continue.dev and Live Server from the VS Code marketplace. Screenshots included.
...
How to open a terminal in VS Code, run `./deploy.sh`, and see the site go live at
`https://<your-slug>.${DOMAIN}`. Common errors and how to fix them.
